...
const SimpleUserDashboard = ({ user, onLogout }) => {
  ...
  const handleStatisticsNav = () => {
    if (role === 'statistics_officer') {
      navigate('/statistics-dashboard');
    } else {
      navigate('/statistics-dashboard'); // or keep current behavior
    }
  };

  const handlePlansNav = () => {
    if (role === 'planning_officer') {
      navigate('/planning-dashboard');
    } else if (role === 'head_of_division') {
      navigate('/head-of-division-dashboard');
    } else if (role === 'head_of_department') {
      navigate('/head-of-department-dashboard');
    } else {
      navigate('/planning-dashboard'); // default fallback
    }
  };
  ...
  const responsiveStyle = `
    ...
    @media (max-width: 600px) {
      ...
      .welcome-card, .stat-card, .action-card, .recent-activity, .approvals-section {
        min-width: 0 !important;
        width: 100% !important;
      ...
      <div className="user-dashboard">
        <div className="dashboard-sidebar">
          ...
          <nav className="sidebar-nav">
            <ul>
              <li>Dashboard</li>
              <li onClick={handleStatisticsNav} style={{ cursor: 'pointer' }}>Statistics</li>
              <li onClick={handlePlansNav} style={{ cursor: 'pointer' }}>Plans</li>
